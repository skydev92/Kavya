...
def download_docs_from_repo(repo_url, repo_name, docs_path, target_dir, branch="main"):
    ...
    try:
        ...
        subprocess.run(
            ["git", "remote", "add", "origin", repo_url], cwd=temp_dir, check=True
        )
        subprocess.run(
            ["git", "config", "core.sparseCheckout", "true"], cwd=temp_dir, check=True
        )
        ...
        subprocess.run(
            ["git", "pull", "--depth=1", "origin", branch], cwd=temp_dir, check=True
        )
...
def download_docs():
    ...
    try:
        ...
        subprocess.run(
            ["git", "pull", "--depth=1", "origin", f"v{version}"],
            cwd=str(temp_dir),
            check=True,
        )
...
def clone_and_copy_docs(
    repo_url, repo_name, source_dir, dest_name, branch="main", specific_files=None
):
