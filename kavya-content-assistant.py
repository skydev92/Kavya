...
def get_content_strategy(request: ContentRequest) -> ContentStrategy:
...
def get_html_strategy(allowed_html_tags: str, content_strategy: ContentStrategy) -> HTMLTagStrategy:
...
def get_content_outline(content_strategy: ContentStrategy, html_strategy: HTMLTagStrategy) -> ContentOutline:
...
def get_content_draft(content_outline: ContentOutline, content_strategy: ContentStrategy, html_strategy: HTMLTagStrategy, style_requirements: str, current_word_count: int = 0) -> ContentDraft:
    ...
    log_instruction("Content Writer", content_writer_prompt)
    log_to_console("Content Writer", "Context", json.dumps({
        "content_outline": content_outline.model_dump(),
        "content_strategy": content_strategy.model_dump(),
        "html_strategy": html_strategy.model_dump(),
    ...
    response = client.chat.completions.create(
        model=MODEL,
        messages=[
            {"role": "system", "content": dedent(content_writer_prompt)},
...
async def content_creation_agent(request: ContentRequest):
    try:
        ...
        while total_word_count < content_strategy.recommended_word_count * 0.9:  # Allow 10% margin
            additional_content = get_content_draft(content_outline, content_strategy, html_strategy, request.style_requirements, total_word_count)
