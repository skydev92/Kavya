...
response=$(curl -s --max-time 300 -w "HTTPSTATUS:%{http_code}" -X POST "http://localhost:8089/v1/chat/completions" \
  ...
  -d '{
    ...
    "messages": [
      {
        "role": "system",
        "content": "You are a web copywriter responding with HTML snippets to be inserted into the body field of a webpage. Do not answer with full HTML pages, navigation, etc. only the body of a blog post."
      },
      {
        "role": "user",
