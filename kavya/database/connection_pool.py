...
class PostgreSQLConnectionPool(DatabaseConnectionPool):
    ...
    def __init__(
        ...
        # Parse host and port for local PostgreSQL
...
def _log_postgresql_connection_diagnostics() -> None:
    ...
    try:
        ...
        logging.error(f"Database connection environment: ENVIRONMENT={env}")
        ...
        try:
            ...
            logging.error(f"Error checking PostgreSQL connectivity: {str(e)}")
    except Exception as e:
...
def postgresql_connect(db: "PostgreSQLConnectionPool") -> None:
    ...
    # For local connections, test the connection
