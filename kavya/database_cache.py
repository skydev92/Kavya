...
class DatabaseCache:
    ...
    def check_sufficient_balance(
        ...
        logging.info(f"=== CHECKS ACCOUNT IN CACHE === account_id: {account_id}")
        ...
        logging.info(
            "DATA AGE IN SECONDS : " + str(time.time() - current_balance["creation"])
        )
    ...
    def update_usage_with_response(
        ...
        cache_start_msg += f"=================================="
        logging.info(
            f"\033[32m{cache_start_msg}\033[0m"
        )  # Using \033[32m for bright green

        current_usage_updates = self.cache["usage_updates"].get(
            account_id,
            {
                "prompt_tokens": 0,
                "completion_tokens": 0,
        ...
        self.cache["usage_updates"][account_id] = {
            ...
            "completion_tokens": current_usage_updates["completion_tokens"]
            + completion_tokens,
        ...
        cache_msg += (
            f"Total Cached Prompt Tokens: {current_usage_updates['prompt_tokens']}\n"
        )
