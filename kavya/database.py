...
class PostgreSQLConnection(DatabaseConnection):
    def __init__(self, database_url, instance_connection_name=None, private_ip=False):
        ...
        self.retry_backoff = float(
            os.getenv("DB_RETRY_BACKOFF", str(DEFAULT_RETRY_BACKOFF))
        )
        self.connect_timeout = int(
            os.getenv("DB_CONNECT_TIMEOUT", str(DEFAULT_CONNECT_TIMEOUT))
        )
        self.command_timeout = int(
            os.getenv("DB_COMMAND_TIMEOUT", str(DEFAULT_COMMAND_TIMEOUT))
        )
        self.lock_timeout = int(os.getenv("DB_LOCK_TIMEOUT", str(DEFAULT_LOCK_TIMEOUT)))
        self.statement_timeout = int(
            os.getenv("DB_STATEMENT_TIMEOUT", str(DEFAULT_STATEMENT_TIMEOUT))
        )
        self.token_update_retries = int(
            os.getenv("DB_TOKEN_UPDATE_RETRIES", str(DEFAULT_TOKEN_UPDATE_RETRIES))
        )
        self.token_update_backoff = float(
            os.getenv(
                "DB_TOKEN_UPDATE_BACKOFF_BASE", str(DEFAULT_TOKEN_UPDATE_BACKOFF_BASE)
            )
        )
        self.idle_in_transaction_timeout = int(
            os.getenv(
                "DB_IDLE_IN_TRANSACTION_TIMEOUT",
                str(DEFAULT_IDLE_IN_TRANSACTION_TIMEOUT),
            )
        )
    ...
    def connect(self):
        ...
        if self.instance_connection_name:  # Production mode with Cloud SQL
            logging.info(
                f"Connecting to Cloud SQL instance: {self.instance_connection_name}"
            )
            connector = Connector(
                refresh_strategy="BACKGROUND"
            )  # Changed from ALWAYS to BACKGROUND
            ...
            def getconn():
                try:
                    ...
                    logging.error(
                        f"Failed to connect to Cloud SQL: {error_type}: {str(e)}"
                    )
            ...
            logging.info(
                f"Connecting to local PostgreSQL at {self.db_host}:{self.db_port}"
            )
        ...
        def handle_error(context):
            ...
            if is_disconnect:
                logging.warning(
                    f"Database connection error detected: {type(error).__name__}: {str(error)}"
                )
            ...
            if is_deadlock:
                ...
                if retries < self.max_retries:
                    ...
                    time.sleep(delay)
                    logging.warning(
                        f"Deadlock detected, retrying (attempt {retries + 1}/{self.max_retries})"
                    )
                    return True
        ...
        try:
            ...
            logging.info(
                f"Database connection established successfully in {time.time() - start_time:.2f}s"
            )
            return self.connection
        except Exception as e:
            error_type = type(e).__name__
            logging.error(
                f"Failed to establish database connection: {error_type}: {str(e)}"
            )
    ...
    def begin_transaction(self):
        ...
        if not self._in_transaction:
            ...
            logging.warning(
                f"⚠️ DB_TXN_ALREADY_ACTIVE: Thread-{thread_id} tried to start transaction but one already active!\n{stack_trace}"
            )
    ...
    def commit(self):
        ...
        if self._in_transaction:
            ...
            if duration > 5.0:  # Log slow transactions
                logging.warning(
                    f"🐌 DB_TXN_SLOW: Thread-{thread_id} SLOW commit after {duration:.2f}s"
                )
            else:
                logging.info(
                    f"✅ DB_TXN_COMMIT: Thread-{thread_id} committed in {duration:.2f}s"
                )
        else:
            logging.warning(
                f"⚠️ DB_TXN_NO_ACTIVE: Thread-{thread_id} tried to commit but no active transaction"
            )
    ...
    def rollback(self):
        ...
        if self._in_transaction:
            try:
                ...
                logging.warning(
                    f"🔄 DB_TXN_ROLLBACK: Thread-{thread_id} rolled back after {duration:.2f}s"
                )
            except Exception as e:
                logging.error(
                    f"💥 DB_TXN_ROLLBACK_FAILED: Thread-{thread_id} rollback error: {type(e).__name__}: {str(e)}"
                )
            ...
            logging.warning(
                f"⚠️ DB_TXN_NO_ROLLBACK: Thread-{thread_id} tried to rollback but no active transaction"
            )
...
class Database:
    ...
    def __init__(self):
        ...
        self._validation_interval = int(
            os.getenv("DB_VALIDATION_INTERVAL", str(DEFAULT_VALIDATION_INTERVAL))
        )

        logging.info(
            f"Initializing database in {'development' if self.env == 'dev' else 'production'} environment"
        )
    ...
    def _get_connection(self, force_new=False, recursion_depth=0):
        ...
        if recursion_depth >= max_recursion:
            logging.error(
                f"💥 DB_CONN_MAX_RECURSION: Thread-{thread_id} failed after {max_recursion} attempts"
            )
            raise RuntimeError(
                f"Failed to establish a valid database connection after {max_recursion} attempts"
            )
        ...
        if force_new or not hasattr(self._local, "db"):
            ...
            try:
                logging.info(
                    f"🔌 DB_CONN_NEW: Thread-{thread_id} creating new database connection"
                )
                ...
                logging.info(
                    f"✅ DB_CONN_READY: Thread-{thread_id} connection established"
                )
            except Exception as e:
        ...
        current_time = time.time()
        if (
            not hasattr(self._local, "last_validation")
            or current_time - self._local.last_validation > self._validation_interval
        ):
            try:
                if not self._local.db.validate():
                    ...
                    return self._get_connection(
                        force_new=True, recursion_depth=recursion_depth + 1
                    )
    ...
    def get_validated_connection(self):
        ...
        while attempt < max_attempts:
            try:
                ...
                logging.warning(
                    f"Connection validation failed (attempt {attempt+1}/{max_attempts}): {type(e).__name__}: {str(e)}"
                )
    ...
    def get_transaction(self):
        ...
        start_time = time.time()
        logging.info(
            f"🚀 DB_CTX_START: Thread-{thread_id} starting context {transaction_id}"
        )
        ...
        try:
            ...
            try:
                ...
                if duration > 10.0:  # Log very slow contexts
                    logging.warning(
                        f"🐌 DB_CTX_VERY_SLOW: Thread-{thread_id} context {transaction_id} took {duration:.2f}s!"
                    )
                elif duration > 5.0:  # Log slow contexts
                    logging.warning(
                        f"🐌 DB_CTX_SLOW: Thread-{thread_id} context {transaction_id} took {duration:.2f}s"
                    )
                else:
                    logging.info(
                        f"✅ DB_CTX_SUCCESS: Thread-{thread_id} context {transaction_id} completed in {duration:.2f}s"
                    )
            except Exception as commit_error:
                error_type = type(commit_error).__name__
                error_msg = str(commit_error)
                logging.error(
                    f"Commit failed for transaction {transaction_id}: {error_type}: {error_msg}"
                )
                ...
                try:
                    db.rollback()
                    logging.debug(
                        f"Transaction {transaction_id} rolled back after commit failure"
                    )
                except Exception:
                    pass
                ...
                if is_transaction_aborted and hasattr(self._local, "db"):
                    try:
                        ...
                        delattr(self._local, "db")
                        logging.warning(
                            f"Connection invalidated after aborted transaction {transaction_id}"
                        )
                    except Exception:
                        pass
            ...
            duration = time.time() - start_time
            logging.error(
                f"💥 DB_CTX_FAILED: Thread-{thread_id} context {transaction_id} failed after {duration:.2f}s: {type(e).__name__}: {str(e)}"
            )
            ...
            error_details = str(e)
            is_transaction_aborted = (
                "current transaction is aborted" in error_details.lower()
                or "25P02" in error_details
            )
            ...
            try:
                ...
                logging.error(
                    f"Error during rollback of transaction {transaction_id}: {type(rollback_error).__name__}: {str(rollback_error)}"
                )
            ...
            is_network_error = (
                ...
                or "network error" in error_details.lower()
            )

            # Log the exact error type and representation
            logging.error(
                f"Transaction {transaction_id} network error - Type: {error_type}, Repr: {repr(e)}"
            )
            ...
            try:
                ...
                logging.error(
                    f"Connection state for transaction {transaction_id}: {conn_info}"
                )
            except Exception:
                pass
            ...
            if self.env == "prod":
                ...
                instance_name = os.getenv("INSTANCE_CONNECTION_NAME", "Not set")
                logging.error(
                    f"Environment context for transaction {transaction_id}: ENVIRONMENT={env}, INSTANCE_CONNECTION_NAME={instance_name}"
                )
    ...
    def _update_balance(
        ...
        try:
            ...
            logging.warning(
                f"[ID: {transaction_id}] Account creation attempt failed: {str(e)}"
            )
        ...
        if not updated_result:
            ...
            if balance:
                ...
                if (
                    balance[0] < prompt_tokens
                    or balance[1] < completion_tokens
                    or balance[2] < word_count
                ):
                    logging.error(
                        f"Insufficient balance for account {account_id}: has ({balance[0]}, {balance[1]}, {balance[2]}), needs ({prompt_tokens}, {completion_tokens}, {word_count})"
                    )
                    raise RuntimeError(
                    ...
                    raise RuntimeError(
                        "Update failed but account exists and has sufficient balance, will retry"
                    )
            else:
    ...
    def update_usage_with_response(
        ...
        start_time = time.time()
        logging.info(
            f"🎯 DB_UPDATE_START: Thread-{thread_id} [ID: {transaction_id}] account={account_id}, prompt={prompt_tokens}, completion={completion_tokens}, words={word_count}"
        )
        ...
        while retry_count < max_retries:
            try:
                ...
                with self.get_transaction() as db:
                    final_balance = self._update_balance(
                        db.get_cursor(),
                        account_id,
                        prompt_tokens,
                        completion_tokens,
                    ...
                    if duration > 10.0:
                        logging.warning(
                            f"🐌 DB_UPDATE_VERY_SLOW: Thread-{thread_id} [ID: {transaction_id}] completed in {duration:.2f}s! Final balance: {final_balance}"
                        )
                    elif duration > 5.0:
                        logging.warning(
                            f"🐌 DB_UPDATE_SLOW: Thread-{thread_id} [ID: {transaction_id}] completed in {duration:.2f}s. Final balance: {final_balance}"
                        )
                    else:
                        logging.info(
                            f"✅ DB_UPDATE_SUCCESS: Thread-{thread_id} [ID: {transaction_id}] completed in {duration:.2f}s. Final balance: {final_balance}"
                        )
                ...
                error_data = Database._check_error_message(
                    error_type, error_msg, backoff_base, retry_count
                )
                ...
                if is_retryable and retry_count < max_retries:
                    ...
                    logging.error(
                        f"[ID: {transaction_id}] Account has insufficient balance: {error_msg}"
                    )
                    raise RuntimeError(f"Insufficient token balance: {error_msg}")
                else:
                    # For other errors or if we've exhausted retries, raise the error
                    logging.error(
                        f"[ID: {transaction_id}] CRITICAL: Failed to update token usage in database: {error_type}: {error_msg}"
                    )
                    raise RuntimeError(
                        f"Failed to update token usage: {error_msg}"
                    ) from e
    ...
    def check_sufficient_balance(
        ...
        while retry_count < max_retries:
            try:
                ...
                try:
                    ...
                    logging.debug(
                        f"[ID: {transaction_id}] Account creation attempt (non-critical): {str(e)}"
                    )
                ...
                word_balance = (
                    ...
                    else self.DEFAULT_WORD_BALANCE
                )
                ...
                if is_retryable and retry_count < max_retries:
                    logging.warning(
                        f"[ID: {transaction_id}] Balance check {error_category} error (attempt {retry_count}/{max_retries}): {error_type}: {error_msg}"
                    )
                    ...
                    logging.warning(
                        f"[ID: {transaction_id}] Assuming sufficient balance after error"
                    )
                    return True, {
                        "token_balance_in": self.DEFAULT_TOKEN_BALANCE_IN,
                        "token_balance_out": self.DEFAULT_TOKEN_BALANCE_OUT,
        ...
        logging.warning(
            f"[ID: {transaction_id}] Assuming sufficient balance after {max_retries} failed attempts"
        )
        return True, {
            "token_balance_in": self.DEFAULT_TOKEN_BALANCE_IN,
            "token_balance_out": self.DEFAULT_TOKEN_BALANCE_OUT,
    ...
    def initialize_test_accounts(self, account_ids: list[int]):
        ...
        if self.env != "dev":
            raise RuntimeError(
                "Test account initialization is only allowed in development environment"
            )
        ...
        try:
            with self.get_transaction() as connection:
                ...
                logging.info(
                    f"Initialized {len(account_ids)} test accounts with default balances"
                )
